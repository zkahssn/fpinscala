...
object PolymorphicFunctions {
  ...
  val numbers = List(1, 2, 3, 4, 5, 6, 7, 7)
  def checkSorted(i: Int, j: Int) = i < j

  def isSorted[A](as: Array[A], gt: (A, A) => Boolean): Boolean = {
    val first = as.head
    val second = as.tail.head
    val size = as.size
    size match {
      case 2 => gt(first, second)
      case _ => if (!gt(first, second)) false else isSorted(as.tail.tail, gt)
    }
